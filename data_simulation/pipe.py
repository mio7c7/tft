...
def adjust_volume_pressure(og, transaction_df, leak_rate, start_date, stop_date, reversedSC, tck):
    ...
    errored_transaction = transaction_df[(transaction_df['pump_start_DN'] >= start_date) & (transaction_df['pump_stop_DN'] <= stop_date)]
    errored_transaction = errored_transaction.sort_values(by='pump_start_DN')

    cum_var, cum_var_tc, trans_id = 0, 0, 0
    trans_time, last_ts = 0, 0
    for idx, row in leaking.iterrows():
        end_time = row['Time_DN']
        while errored_transaction.iloc[trans_id]['pump_start_DN'] < end_time:
            trans_id += 1
            if trans_id >= len(errored_transaction):
                break
        copy = errored_transaction.iloc[:trans_id]
        for copyidx, copyrow in copy.iterrows():
            if copyrow['pump_start_DN'] > last_ts:
                trans_time += copyrow['pump_stop_DN'] - copyrow['pump_start_DN']
                last_ts = copyrow['pump_stop_DN']
            else:
                trans_time += copyrow['pump_stop_DN'] - last_ts
                last_ts = copyrow['pump_stop_DN']
        if last_ts > end_time:
            trans_time -= last_ts - end_time
        leakage = -0.4*GAL*trans_time/3600 - 0.2*GAL*(1800-trans_time)/3600
        ...
        if last_ts > end_time:
            ...
            trans_time = 0
        errored_transaction = errored_transaction.iloc[trans_id:]
        trans_id = 0
...
def adjust_volume_suction(og, transaction_df, leak_rate, start_date, stop_date, reversedSC, tck):
    def interpolate_height(volume):
        for i, model in enumerate(reversedSC):
            if volumes[i] <= volume <= volumes[i + 1]:
                return np.polyval(model, volume)
    ...
    for idx, row in errored_transaction.iterrows():
        leakage = - 0.2 * GAL * (row['pump_stop_DN'] - row['pump_start_DN']) / 3600
        leakage_tc = leakage * (1 + tck * (15 - leaking.at[idx, 'TankTemp']))
        cum_var += leakage
        cum_var_tc += leakage_tc

        try:
            matching_row = leaking[(leaking['Time_DN'].shift(1) <= row.pump_stop_DN ) & (leaking['Time_DN'] > row.pump_stop_DN )].iloc[-1]
        except:
...
for i in glob.glob(folder):
    ...
    inventory_file = [filename for filename in os.listdir(site_dir) if filename.endswith('.csv') and '_inventories_' in filename]
    ...
    inventory_30ms = fs_30mins.merge(inventory_df[['event_time', 'tc_volume', 'height']], left_on='Time', right_on='event_time', how='left')
    new_column_names = {
    ...
    induced_df = adjust_volume_pressure(inventory_30ms, transaction_df, rate, start_date, stop_date, reversedSC, tck)
    induced_df = assign_period(induced_df)
    final = df_format(induced_df)

    file_name = Site + '_' + tank + "_" + str(rate) + '_' + str(errored_pump) + '_Pipe'
    ...
    simulate_info.loc[C] = [Site, tank, rate, start_date, errored_pump, stop_date, file_name]
    C += 1
    simulate_info.to_csv('pressure_info.csv')
