...
# tlgrouths = pd.read_csv('C:/Users/s3912230/Documents/GitHub/tft/data_simulation/tankleakage_info.csv', index_col=0).reset_index(drop=True)test_sequence = pd.read_csv('tankleak.csv')
TRAINSIZE = 4000
VALIDSIZE = 500
tt = test_sequence[lambda x: x.time_idx <= TRAINSIZE + VALIDSIZE]
tt = tt[(tt['Var_tc_readjusted'] < 1) & (tt['Var_tc_readjusted'] > -1)]
processed_dfs = []
groups = tt.groupby('group_id')
